# Ajay Lakkuntla

🎓 Master's in Business Analytics – Webster University  📍 St. Louis, Missouri, USA  
📊 Business Analyst | Data Analyst | Data Engineer | Supply Chain Analytics | Construction Business Analytics  

---

## 👨‍💻 About Me

I am a **Business Analytics graduate student at Webster University** with experience in **data analysis, database management, business intelligence, and ERP systems**. I focus on transforming raw data into meaningful insights that support **business strategy, operational efficiency, and data-driven decision making**.

My background combines **analytics, machine learning, ERP systems, and business process analysis**, allowing me to work across industries such as **business intelligence, supply chain analytics, infrastructure analytics, and construction analytics**.

I enjoy solving problems through **data analysis, visualization, predictive modeling, and business process optimization**.

---

# 🛠 Technical Skills

### Data Analytics & Visualization
- Power BI
- Tableau
- Excel
- Data Visualization
- KPI Dashboards
- Business Intelligence Reporting

### Programming & Analytics
- Python
- R
- Statistical Analysis
- Predictive Modeling
- Machine Learning

### Data Engineering & Databases
- SQL
- MySQL
- Oracle
- Data Warehousing
- ETL Processes
- Data Pipelines

### Business Systems & Platforms
- ERP Systems
- SAP
- Business Process Analysis
- Data Integration

### Engineering & Construction Tools
- AutoCAD
- Revit
- STAAD Pro
- Infrastructure Data Analysis

### Tools & Platforms
- AWS
- GitHub
- Power BI
- Tableau
- Python
- R

---

# 🎓 Relevant Coursework – Master’s in Business Analytics  
*Webster University*

## 📊 Analytics & Machine Learning
- **CSBU 5420** – Machine Learning for Business Analytics  
- **CSDA 5330** – Machine Learning for Predictive Analytics  
- **CSDA 5110** – Analytics Programming with R  
- **CSDA 5320** – Analytics Applications using Python  

## 🗄 Data Management & Databases
- **CSBU 5300** – Database Systems for Business Analytics  
- **CSDA 5210** – Databases and Data Warehouses  
- **CSDA 5240** – Database Programming  

## 📈 Data Analytics & Visualization
- **CSBU 5320** – Data Analytics Foundations for Business Analytics  
- **CSDA 5310** – Data Visualization  
- **BUSN 5760** – Applied Business Statistics  

## 🏢 ERP & Business Systems
- **CSBU 5340** – ERP Systems and Business Transformation  
- **BUSN 6160** – Integrated Business Processes and ERP  
- **CSBU 6300** – Descriptive Analytics and ERP Integration Practicum  

## 💼 Business Applications
- **MRKT 5895** – Marketing Analytics  
- **BUSN 5200** – Basic Finance for Managers  
- **CSDA 5130** – Social and Ethical Issues in Analytics  

---

# 🚀 Tools & Technologies

![Python](https://img.shields.io/badge/Python-Analytics-blue)
![R](https://img.shields.io/badge/R-Statistical%20Computing-blue)
![SQL](https://img.shields.io/badge/SQL-Database-orange)
![PowerBI](https://img.shields.io/badge/PowerBI-Data%20Visualization-yellow)
![Tableau](https://img.shields.io/badge/Tableau-Analytics-blue)
![AWS](https://img.shields.io/badge/AWS-Cloud-orange)
![SAP](https://img.shields.io/badge/SAP-ERP-blue)
![Excel](https://img.shields.io/badge/Excel-Data%20Analysis-green)
![AutoCAD](https://img.shields.io/badge/AutoCAD-Engineering-red)
![Revit](https://img.shields.io/badge/Revit-BIM-blue)
![STAADPro](https://img.shields.io/badge/STAADPro-Structural%20Engineering-yellow)

---

# 📁 Areas of Interest

- Business Analytics
- Data Analytics
- Business Intelligence
- Data Engineering
- Supply Chain Analytics
- Construction Analytics
- Infrastructure Data Analysis
- ERP Systems & Business Transformation
- Machine Learning for Business

---

# 📬 Connect With Me

💼 LinkedIn  
https://www.linkedin.com/in/analyst-ajay/

💻 GitHub  
https://github.com/uh-JAY

---

⭐ Always open to collaborating on **data analytics, business intelligence, and data-driven projects**.
## 📂 Featured Files

### 1️⃣ Final Project Analysis
**File:** [`Final_Project_Analysis.xlsx`](Final_Project_Analysis.xlsx)  
**Description:**  
An Excel-based project analyzing business performance metrics, using data visualization, pivot tables, and key KPI calculations.

### 2️⃣ Marketing Report
**File:** [`Marketing_Report.docx`](Marketing_Report.docx)  
**Description:**  
A detailed report documenting marketing insights and recommendations developed from data-driven analysis in my final coursework.

---

## 🧠 My Learning Focus
I’m passionate about:
- Integrating data analysis into real-world business decision-making  
- Understanding ERP systems and business process optimization  
- Building predictive and prescriptive analytics models  

---

## 🔗 Connect with Me
📧 **Email:** ajaylakkuntla21@gmail.com  
🌐 **Portfolio:** [https://uh-jay.github.io/portfolio/](https://uh-jay.github.io/portfolio/)  
💼 **LinkedIn:** [www.linkedin.com/in/ahjay-da](www.linkedin.com/in/ahjay-da)
🐙 **GitHub:** [https://github.com/uh-JAY](https://github.com/uh-JAY)

---

> “Data tells stories — my goal is to listen, translate, and transform them into business value.”
